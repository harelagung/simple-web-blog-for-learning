...
<div class="relative p-4 bg-white rounded-lg border dark:bg-gray-800 sm:p-5">
    ...
    <!-- Modal body -->
    <form action="/dashboard/store" method="POST" id="post-form">
        @csrf
        <div class="grid gap-4 mb-4 sm:grid-cols-2">
            ...
            <div>
                <label for="category"
                    ...
                    @foreach ($category as $c)
                        <option value="{{ $c->id }}" @selected(old('category') == $c->id)>{{ $c->name }}</option>
                    @endforeach
                </select>
                @error('category')
                    <p class="mt-2 text-sm text-red-600 dark:text-red-500 text-xs">{{ $message }}</p>
            ...
            <div class="sm:col-span-2">
                ...
                <textarea name='body' id="body"
                    ...
                    placeholder="Write post body ...">{{ old('body') }}</textarea>
        ...
        <div class="flex space-x-4">
            <button type="submit"
                class="z-50 text-white inline-flex items-center bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800 cursor-pointer">
